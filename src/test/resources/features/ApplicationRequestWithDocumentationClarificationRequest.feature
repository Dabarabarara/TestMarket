@smoke @runTest @application_request_with_documentation_clarification
Feature: Application Request With Documentation Clarification Request
  #=====================================================================================================================
  # Конкурентный запрос предложений - Запрос на разъяснение документации через форму извещения
  # ( https://ve-lab.visualstudio.com/kontur.market/_workitems/edit/345650 )
  ...
  Scenario: Конкурентный запрос предложений с запросом на разъяснение документации
    Given Параметру 'Тип теста' установлено значение "Application Request With Documentation Clarification Request"
    ...
    And 'Заказчик А' открывает страницу создания извещения для конкурентной закупки "Запрос предложений"
    And 'Заказчик А' заполняет сведения о процедуре: "Конкурентная закупка по 223ФЗ - Запрос предложений"
    #
    ...
    And 'Заказчик А' заполняет поле 'Наименование закупки' сгенерированным именем для типа закупки "Запрос предложений"
    And 'Заказчик А' заполняет поле "Описание закупки" значением "Автоматические тесты - Конкурентная закупка - Запрос предложений"
    ...
    #-------------------------------------------------------------------------------------------------------------------
    #                 Заказчик дает ответ на запрос на разъяснение документации через форму извещения
    #-------------------------------------------------------------------------------------------------------------------
    And 'Заказчик' переходит на страницу 'Мои закупки' и ищет опубликованное извещение
    ...
    And 'Заказчик' нажимает на кнопку "Ответить" строка 1 в таблице 'Запросы на разъяснение документации и ответы'
