@smoke @runTest @uncomp_appl_req_ws_single_participant_and_retendering
Feature: Uncompetitive Application Request With Single Participant And Retendering
  #=====================================================================================================================
  # Проведение неконкурентного запроса предложений с возможностью проведения переторжки с 1 участником
  # ( https://ve-lab.visualstudio.com/kontur.market/_workitems/edit/355118 )
  ...
  #=====================================================================================================================
  Scenario: Запрос предложений с возможностью проведения переторжки с 1 участником
    Given Параметру 'Тип теста' установлено значение "Application Request With One Participant And Retendering"
    ...
    And 'Заказчик' проверяет что флаг "Флаг Возможно проведение переторжки с одним участником" на странице 'Общие настройки' "включен"
    #*******************************************************************************************************************
    #
    #                 Заказчик публикует извещение о проведении конкурентного конкурса
    #
    #*******************************************************************************************************************
    And 'Заказчик' переходит на страницу "ЗАКУПКИ/Заказчикам"
    And 'Заказчик А' открывает страницу создания извещения для неконкурентной закупки "Запрос предложений"
    And 'Заказчик А' заполняет сведения о процедуре: "Закупка по 223-ФЗ - Запрос предложений"
    #
    #                 Общие сведения о закупке
    #
    And 'Заказчик А' переходит к разделу "Общие сведения о закупке":
    And 'Заказчик А' проверяет, что поле "Способ закупки" содержит значение "Запрос предложений"
    And 'Заказчик А' проверяет, что поле "Тип закупки" содержит значение "Закупка в соответствии с нормами 223-ФЗ"
    And 'Заказчик А' заполняет поле 'Наименование закупки' сгенерированным именем для типа закупки "Запрос предложений - ЗП с одним уч."
    And 'Заказчик А' заполняет поле "Описание закупки" значением "Автоматические тесты - Неконкурентная закупка - ЗП с одним уч."
    ...
    And 'Заказчик А' выбирает значение "Фиксированная сумма" в списке для поля 'Вид обеспечения заявки'
    And 'Заказчик А' выбирает значение "Фиксированная сумма" в списке для поля 'Вид обеспечения договора'
    And 'Заказчик А' выбирает второе по счёту значение в списке для поля 'Заказчик'
    And 'Заказчик А' проверяет, что поле "Обеспечение заявки (%)" не доступно для редактирования
    And 'Заказчик А' проверяет, что поле "Обеспечение договора (%)" не доступно для редактирования
    And 'Заказчик А' заполняет поле "Обеспечение заявки (RUB)" значением "1"
    And 'Заказчик А' заполняет поле "Обеспечение договора (RUB)" значением "1"
    And 'Заказчик А' переходит к полю "Место поставки товара, выполнения работ, оказания услуг"
    And 'Заказчик А' проверяет, что поле "Обеспечение заявки (%)" содержит значение "20,00"
    And 'Заказчик А' проверяет, что поле "Обеспечение договора (%)" содержит значение "20,00"
    ...
    And 'Администратор' выбирает "Обновить статусы лотов запросов предложений"
    And 'Пользователь' покидает систему
    #-------------------------------------------------------------------------------------------------------------------
    #                 Заказчик проверяет статус закупки и статус лота
    #-------------------------------------------------------------------------------------------------------------------
    ...
    And 'Заказчик' добавляет к протоколу "рассмотрения и оценки заявок" переторжку с единственным участником
    ...
    Then Ценовое предложение отобразилось в списке лучших ценовых предложений "4"
    And 'Поставщик' заполняет поле "Предложение ставки" значением "3,5" на странице 'Торги'
    And 'Поставщик' подает ценовое предложение
    Then Ценовое предложение отобразилось в списке лучших ценовых предложений "3,5"
    And 'Пользователь' покидает систему
    #*******************************************************************************************************************
    #
    ...
    #                 Заказчик проверяет статус закупки и статус лота
    #-------------------------------------------------------------------------------------------------------------------
    # TODO Данный код необходимо раскомментировать для обхода бага с исключением при переходе по ссылке "Направьте договор"
#    And 'Заказчик' переходит на страницу 'Мои закупки' и ищет опубликованное извещение
#    And 'Заказчик' открывает извещение по ссылке с номером извещения в результатах поиска
#    And 'Заказчик' проверяет статус закупки "Опубликована" и статус лота "Заключение договоров" для "1" лотов
    #*******************************************************************************************************************
    #
    #                 Заказчик направляет договор Поставщику
    #
    #*******************************************************************************************************************
    And 'Заказчик' переходит на страницу 'Мои закупки' и ищет опубликованное извещение
    ...
    #*******************************************************************************************************************
    #
    #                 Поставщик подписывает договор с Заказчиком
    #
    #*******************************************************************************************************************
    When "Поставщик Юридическое лицо 1" заходит в личный кабинет
    ...
    #*******************************************************************************************************************
    #
    #                 Заказчик подписывает договор с Поставщиком
    #
    #*******************************************************************************************************************
    When "Заказчик" заходит в личный кабинет
    And 'Заказчик' заполняет поле 'Номер закупки' и нажимает на кнопку 'Поиск'
