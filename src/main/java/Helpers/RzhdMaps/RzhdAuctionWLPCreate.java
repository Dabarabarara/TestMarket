...
/**
 * Класс-контейнер для хранения информации о наборе параметров для файла-шаблона AOE.xml
 * ( создание извещения об аукционе с ограниченным участием ).
 * Created by Vladimir V. Klochkov on 20.05.2019.
 * Updated by Vladimir V. Klochkov on 24.06.2020.
 */
public class RzhdAuctionWLPCreate {
    /*******************************************************************************************************************
     * Поля класса.
     ******************************************************************************************************************/
     ...
     ******************************************************************************************************************/
    /**
     * Возвращает набор параметров для файла-шаблона AOE.xml.
     *
     * @return набор параметров для файла-шаблона AOE.xml
     */
    public HashMap<String, String> getParams() {
        return new HashMap<String, String>() {{
            ...
            //
            put("ns2:name", config.getConfigParameter("RzhdPurchaseNameForAuctionWLPCreate"));
            //
            ...
            put(lr, timer.dateOnlyToResponseFormat(Timer.getPublishNoticeDate()));
            //
            ...
            put(lr, timer.dateOnlyToResponseFormat(Timer.getPublishNoticeDate(), 2));
            //
            // Дата и время окончания подачи заявок (МСК)
            //
            lr = "ns2:submissionCloseDateTime";
            put(lr, timer.dateToResponseFormatWithSeconds(Timer.getPublishNoticeDate(), 3));
            //
            // Место подачи заявок
            //
            put("ns2:applSubmisionPlace", config.getConfigParameter("RzhdApplSubmisionPlaceCreate"));
            //
            // Дата и время вскрытия заявок
            //
            lr = "//integrCode[contains(., 'VskritieData')]/../value/dateTime";
            put(lr, timer.dateToResponseFormatWithSeconds(Timer.getPublishNoticeDate(), 4));
            //
            ...
            put(lr, timer.dateToResponseFormatWithSeconds(Timer.getPublishNoticeDate(), 4));
            //
            // Дата и время проведения аукциона (МСК)
            //
            lr = "//integrCode[contains(., 'ItogDate')]/../value/dateTime";
            put(lr, timer.dateToResponseFormatWithSeconds(Timer.getPublishNoticeDate(), 5));
            //
            // Дата и время подведения итогов (МСК)
            //
            lr = "ns2:summingupDateTime";
            put(lr, timer.dateToResponseFormatWithSeconds(Timer.getPublishNoticeDate(), 6));
            //
            ...
            put(lr, String.format("Rzhd - Auction With Limited Participation %s",
            ...
            put(lr, config.getConfigParameter("RzhdEtzpPricingConditionCreate"));
            //
            ...
            put(lr, config.getConfigParameter("RzhdEtzpContractEndDateCreate"));
            //
            ...
            //
            // Список ИНН участников
            //
            lr = "//integrCode[contains(., 'etzpQualificationParticipants')]/../value/string";
            put(lr, config.getConfigParameter("OnProdSupplier1Inn") + ";" +
                    config.getConfigParameter("OnProdSupplier2Inn") + ";" + "0101010101");
        }};
    }
}
