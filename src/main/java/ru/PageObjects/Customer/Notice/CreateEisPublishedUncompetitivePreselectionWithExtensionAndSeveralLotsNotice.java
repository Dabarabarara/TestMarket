...
/**
 * Этот класс содержит методы для связывания неконкурентной закупки - Предварительный отбор ( с продлениеи) и несколькими
 * лотами с номером от ЕИС с помощью генерации
 * .xml-ответа от ЕИС.
 * Created by Kirill G. Rydzyvylo on 16.07.2020.
 * Updated by Vladimir V. Klochkov on 25.02.2021.
 */
public class CreateEisPublishedUncompetitivePreselectionWithExtensionAndSeveralLotsNotice extends CreateEisPublishedNotice {
    /*******************************************************************************************************************
     * Методы класса.
     ******************************************************************************************************************/
    /**
    ...
    public String updateModel(String model) throws Exception {
        ...
        params.put("{ns2:item:deliveryStartDateTime}", timer.dateOnlyToResponseFormat(publishNoticeDate, 0));
        params.put("{ns2:item:deliveryEndDateTime}", timer.dateOnlyToResponseFormat(publishNoticeDate, 1));
        params.put("{ns2:item:place}",
                "Электронная площадка РТС-тендер (https://kontur.ru/) и Официальный сайт (http://zakupki.gov.ru)");
        params.put("{ns2:item:procedure}", "В электронной форме на площадке РТС-тендер (https://kontur.ru/) и " +
        ...
        params.put("{ns2:item:lotData:subject}",
                "Предмет предварительного отбора " + timer.getPublishNoticeDateInFormatyyyyMMddhhmmss() + " №1");
        ...
        params.put("{ns2:item:lot:guid2}", UUID.randomUUID().toString());
        params.put("{ns2:item:lotData:subject2}",
                "Предмет предварительного отбора " + timer.getPublishNoticeDateInFormatyyyyMMddhhmmss() + " №2");
        params.put("{ns2:item:lot:lotItem:guid2}", UUID.randomUUID().toString());
        params.put("{ns2:item:lotData:initialSum2}", "5.00");
